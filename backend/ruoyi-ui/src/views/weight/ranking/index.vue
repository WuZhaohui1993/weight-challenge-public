  ...
  <div class="app-container">
    ...
    <el-row :gutter="10" class="mb8">
      <el-col :span="1.5">
        <el-button
          ...
          size="mini"
          @click="handleAdd"
          v-hasPermi="['ruoyi-weight:ranking:add']"
        >新增</el-button>
      </el-col>
      <el-col :span="1.5">
        <el-button
          ...
          :disabled="single"
          @click="handleUpdate"
          v-hasPermi="['ruoyi-weight:ranking:edit']"
        >修改</el-button>
      </el-col>
      <el-col :span="1.5">
        <el-button
          ...
          :disabled="multiple"
          @click="handleDelete"
          v-hasPermi="['ruoyi-weight:ranking:remove']"
        >删除</el-button>
      </el-col>
      <el-col :span="1.5">
        <el-button
          ...
          size="mini"
          @click="handleExport"
          v-hasPermi="['ruoyi-weight:ranking:export']"
        >导出</el-button>
      </el-col>
    ...
    <el-table v-loading="loading" :data="rankingList" @selection-change="handleSelectionChange">
      ...
      <el-table-column label="排名" align="center" width="80">
        <template slot-scope="scope">
          <el-tag :type="scope.row.rankNum <= 3 ? 'warning' : 'info'" size="small">
            #{{ scope.row.rankNum }}
          </el-tag>
        </template>
      </el-table-column>
...
<script>
import { listRanking, getRanking, delRanking, addRanking, updateRanking } from "@/api/weight/ranking"
...
export default {
  ...
  methods: {
    ...
    handleDelete(row) {
      ...
      this.$modal.confirm('是否确认删除排行榜缓存编号为"' + ids + '"的数据项？').then(function() {
    ...
    /** 导出按钮操作 */
    handleExport() {
      this.download('ruoyi-weight/ranking/export', {
        ...this.queryParams
