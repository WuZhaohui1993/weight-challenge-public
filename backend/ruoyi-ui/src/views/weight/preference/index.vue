<template>
  <div class="app-container">
    <el-form :model="queryParams" ref="queryForm" size="small" :inline="true" v-show="showSearch" label-width="68px">
      ...
      <el-form-item label="是否自动同步" prop="autoSync">
        <el-input
          v-model="queryParams.autoSync"
          placeholder="请输入是否自动同步"
          clearable
          @keyup.enter.native="handleQuery"
        />
      </el-form-item>
    ...
    <el-row :gutter="10" class="mb8">
      <el-col :span="1.5">
        <el-button
          ...
          size="mini"
          @click="handleAdd"
          v-hasPermi="['ruoyi-weight:preference:add']"
        >新增</el-button>
      </el-col>
      <el-col :span="1.5">
        <el-button
          ...
          :disabled="single"
          @click="handleUpdate"
          v-hasPermi="['ruoyi-weight:preference:edit']"
        >修改</el-button>
      </el-col>
      <el-col :span="1.5">
        <el-button
          ...
          :disabled="multiple"
          @click="handleDelete"
          v-hasPermi="['ruoyi-weight:preference:remove']"
        >删除</el-button>
      </el-col>
      <el-col :span="1.5">
        <el-button
          ...
          size="mini"
          @click="handleExport"
          v-hasPermi="['ruoyi-weight:preference:export']"
        >导出</el-button>
      </el-col>
    ...
    <el-table v-loading="loading" :data="preferenceList" @selection-change="handleSelectionChange">
      ...
      <el-table-column label="是否自动同步" align="center" width="110">
        <template slot-scope="scope">
          <el-tag :type="toFlagTagType(scope.row.autoSync)" size="small">
            {{ toFlagText(scope.row.autoSync, '自动同步', '手动同步') }}
          </el-tag>
        </template>
      </el-table-column>
    ...
    <el-dialog :title="title" :visible.sync="open" width="500px" append-to-body>
      <el-form ref="form" :model="form" :rules="rules" label-width="80px">
        ...
        <el-form-item label="是否自动同步" prop="autoSync">
          <el-select v-model="form.autoSync" placeholder="请选择" style="width: 100%">
            <el-option label="自动同步" value="1" />
...
<script>
import { listPreference, getPreference, delPreference, addPreference, updatePreference } from "@/api/weight/preference"
import weightAdminPageMixin from "@/mixins/weightAdminPage"

export default {
  name: "Preference",
  mixins: [weightAdminPageMixin],
  routeQueryFilterKeys: ['userId', 'circleId'],
  data() {
    return {
      // 遮罩层
      loading: true,
  ...
  methods: {
    ...
    handleDelete(row) {
      ...
      this.$modal.confirm('是否确认删除同步偏好设置编号为"' + ids + '"的数据项？').then(function() {
    ...
    /** 导出按钮操作 */
    handleExport() {
      this.download('ruoyi-weight/preference/export', {
        ...this.queryParams
