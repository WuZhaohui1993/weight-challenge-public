<template>
  <div class="app-container">
    ...
    <el-table v-loading="loading" :data="achievementList" @selection-change="handleSelectionChange">
      ...
      <el-table-column label="徽章类型" align="center" prop="badgeType" min-width="150">
        <template slot-scope="scope">
          <el-tag size="small" type="warning">{{ formatBadgeType(scope.row.badgeType) }}</el-tag>
        </template>
      </el-table-column>
      ...
      <el-table-column label="进度(0-100)" align="center" width="140">
        <template slot-scope="scope">
          <el-progress :percentage="Number(scope.row.progress || 0)" :stroke-width="10" :show-text="true" />
        </template>
      </el-table-column>
      <el-table-column label="是否已解锁" align="center" width="90">
        <template slot-scope="scope">
          <el-tag :type="toFlagTagType(scope.row.isUnlocked)" size="small">
            {{ toFlagText(scope.row.isUnlocked, '已解锁', '未解锁') }}
          </el-tag>
        </template>
      </el-table-column>
    ...
    <el-dialog :title="title" :visible.sync="open" width="500px" append-to-body>
      <el-form ref="form" :model="form" :rules="rules" label-width="80px">
        ...
        <el-form-item label="是否已解锁" prop="isUnlocked">
          <el-select v-model="form.isUnlocked" placeholder="请选择" style="width: 100%">
            <el-option label="已解锁" :value="1" />
            <el-option label="未解锁" :value="0" />
          </el-select>
        </el-form-item>
...
<script>
import { listAchievement, getAchievement, delAchievement, addAchievement, updateAchievement } from "@/api/weight/achievement"
...
export default {
  ...
  data() {
    return {
      ...
      achievementList: [],
      badgeTypeOptions: [
        { label: '新手起步 first-record', value: 'first-record' },
        { label: '连续打卡 streak-week', value: 'streak-week' },
        { label: '补水计划 water-plan', value: 'water-plan' },
        { label: '目标明确 weight-goal', value: 'weight-goal' },
        { label: '习惯养成 habit-builder', value: 'habit-builder' },
        { label: '圈子同行 circle-companion', value: 'circle-companion' }
      ],
      // 弹出层标题
      title: "",
      // 是否显示弹出层
  ...
  methods: {
    ...
    formatBadgeType(value) {
      const option = this.badgeTypeOptions.find(item => item.value === value)
